...
main <- function() {
	...
	for( otuLevel in getProperty("report.taxonomyLevels") ) {
		...
		parInputFile = getPipelineFile( buildStatsFileEnding(parametric=TRUE, adjusted=FALSE, level=otuLevel) )
		...
		nonParInputFile = getPipelineFile( buildStatsFileEnding(parametric=FALSE, adjusted=FALSE, level=otuLevel) )
