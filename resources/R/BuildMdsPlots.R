		...
		for( metaCol in mdsCols )
		{
			...
			for (x in 1:numAxis) {
				for (y in 2:numAxis) {
					...
					plot( myMDS$CA$u[,x], myMDS$CA$u[,y], main=paste( "MDS", attName, otuLevel),
								xlab=getMdsLabel( percentVariance[x] ), ylab=getMdsLabel( percentVariance[y] ), 
								cex=1.2, pch=getProperty("r.pch"), col=getMdsColors( otuTable, metaCol ) )
				}
			}
		}
		if( doDebug() ) sink()
