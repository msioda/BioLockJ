...
public class Pipeline
{
	 ...
	 * <li>Execute {@link #refreshMetadataCache(File)} to update metadata cache if a new metadata file was generated by
	 * the previous module.
	 * <li>Run module scripts, if any, polling 1/minute for status until all scripts complete or time out.
	 * <li>File {@value #BLJ_STARTED} is removed to the module directory
	 * <li>File {@value #BLJ_COMPLETE} is added to the module directory
	 * </ol>
	 *
	...
	protected static void executeModules() throws Exception
	{
		...
		for( final BioModule module: Pipeline.getModules() )
		{
			if( !ModuleUtil.isComplete( module ) )
			{
				...
				refreshMetadataCache( getMetadata( prevModule ) );
				module.executeTask();
				...
				if( RuntimeParamUtil.isDockerMode() || isJava && runScripts )
				{
					refreshMetadataCache( getMetadata( module ) );
				}
	...
	{
		if( bioModule != null && ModuleUtil.isComplete( bioModule ) )
		{
	...
	{
		for( final BioModule bioModule: getModules() )
		{
			final String moduleDir = ModuleUtil.getModuleRootDir( bioModule );
			bioModule.setModuleDir( moduleDir );

			if( !RuntimeParamUtil.isDirectMode() && ModuleUtil.isIncomplete( bioModule ) || bioModule instanceof Email )
			{
				...
				bioModule.setModuleDir( moduleDir ); // this method rebuilds deleted directory
			}

			if( ModuleUtil.isComplete( bioModule ) )
			{
				info( "Run cleanUp for: " + bioModule.getClass().getName() );
				bioModule.cleanUp();
			}
			else if( !RuntimeParamUtil.isDockerMode() || DockerUtil.isManager() || RuntimeParamUtil.isDirectMode()
					&& RuntimeParamUtil.getDirectModule().equals( bioModule.getClass().getName() ) )
			{
				info( "Check module dependencies for: " + bioModule.getClass().getName() );
				bioModule.checkDependencies();
			}

			if( getMetadata( bioModule ) != null )
			{
				info( "Reload BioLockJ generated metadata: " + getMetadata( bioModule ).getAbsolutePath() );
				refreshMetadataCache( getMetadata( bioModule ) );
			}
		}
	...
	/**
	 * Call {@link biolockj.util.MetaUtil} to refresh the metadata cache.
	 * 
	 * @param metadata Metadata file
	 * @throws Exception if unable to refresh the cache
	 */
	protected static void refreshMetadataCache( final File metadata ) throws Exception
	{
		if( metadata != null )
		{
