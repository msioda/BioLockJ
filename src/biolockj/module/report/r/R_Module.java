...
public abstract class R_Module extends ScriptModuleImpl {
	...
	public void checkColorFile() throws ConfigPathException, IOException, ConfigViolationException {
		File colorFile = Config.getExistingFile( this, R_COLOR_FILE );
		if (!( colorFile==null )) {
			BufferedReader br = new BufferedReader( new FileReader( colorFile ) );
			String[] header = br.readLine().split( Constants.TAB_DELIM );
			br.close();
			if( ! header[0].equals( "key" ) || ! header[1].equals( "color" )) {
				throw new ConfigViolationException( R_COLOR_FILE, 
						"The color reference file should be a tab-delimited file, with two labeled columns: \"key\" and \"color\".\n" +
	...
	/**
	 * {@link biolockj.Config} property {@value #R_COLOR_FILE} gives the path to a tab-delimited file giving the
	 * color to use for each value of each metadata filed plotted.
	 */
