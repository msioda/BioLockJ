...
public class QiimeClassifier extends ClassifierModuleImpl {
	...
	private void verifyParamArg() throws Exception {
		for( String arg: PARAM_ARGS )
			for( String param: getClassifierParams() )
				if( param.startsWith( arg + " " ) ) {
					...
					if( !paramFile.isFile() ) throw new ConfigPathException( paramFile, "Property \"" + QIIME_PARAMS + "\" contains an invalid file path for -p parameter" );
