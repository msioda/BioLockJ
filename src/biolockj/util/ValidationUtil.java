 ...
 * expectation file, the utility reports the values for these attributes and saves a file in the validation folder which
 * can be used as an expectation in future runs. If there is an expectation file, the values for the current outputs
 * will be compared with the expected values and the utility will stop the pipeline if the files do not meet
 * expectations. Set {@value #HALT_PIPELINE} to Y to stop the pipeline if expectations are not met. Be default
 * ({@value #HALT_PIPELINE}=N), the utility will report on the comparison but not stop the pipeline.<br>
 * <br>
 * 
 * This is an easy way to verify that the outputs of a pipeline are the same as a previous run, which is particularly
 * helpful after software has been updated, or after any other changes that are not expected to alter results.<br>
 * <br>
 * 
 * To provide an expectation file, use {@link biolockj.Config} property {@value #EXPECTATION_FILE} and give the file
 * path to the validation folder of the run that you want to match. To specify which file attributes are reported, use
 * {@link biolockj.Config} property {@value #REPORT_ON}. To specify which file attributes are used for the comparison,
 * use use {@link biolockj.Config} property {@value #COMPARE_ON}.<br>
 * <br>
 */
public class ValidationUtil {
	...
	private static class FileSummary {
		...
		public int compareToExpected( final FileSummary other, final Collection<String> comparisons,
			...
			for( final String att: availableAttributes )
		...
		public String toString() {
			return "FileSummary [file=" + this.file + ", name=" + this.name + ", size=" + this.size +
				", validationStatus=" + statusStrings[ this.validationStatus ] + "]";
		}
		...
		protected String getAtt( final String col ) throws ValidationException {
			switch( col ) {
				case NAME:
					return getName();
				case SIZE:
				...
				case MATCHED_EXPECTATION:
					return statusStrings[ this.validationStatus ];
				default:
					throw new ValidationException( "Validation attributes must be \"" + NAME + "\", \"" + SIZE +
						"\", \"" + MD5 + "\", or \"" + MATCHED_EXPECTATION + "\"" );
			}
		}

		protected void setAtt( final String col, final String val ) throws ValidationException {
			...
			switch( col ) {
				...
				default:
					throw new ValidationException(
						"Unalbe to set validation attribute [ " + col + "]  must be \"" + SIZE + "\", or \""+ MD5 + "\"" );
			}
		}
		...
		private boolean passesSoftValidation( final FileSummary other, final BioModule module )
			...
			if( getSizePercentAllowed( module ) > 0 ) {
				...
				if( Math.abs( percentDiff ) <= getSizePercentAllowed( module ) ) {
					Log.info( this.getClass(),
						"File [" + this.name + "] passes soft size validation: " + getSize() + " is within " +
							getSizePercentAllowed( module ) + "% of the expected value: " + other.getSize() + " ." );
					return true;
				}
			}
			return false;
		...
		private int validationStatus = REPORT;
	...
	/**
	 * Return the validation directory under the pipeline root dir
	 * 
	 * @return Validation file directory
	 */
	...
	/**
	 * Primary API method to validate the given BioModule
	 * 
	 * @param module BioModule
	 * @throws ValidationException If validation fails
	 */
	public static void validateModule( final BioModule module ) throws ValidationException {
		try {
			if( !Config.getBoolean( module, DISABLE_VALIDATION ) ) {
			...
			Log.error( ValidationUtil.class, "ValidationUtil Failure!", ex );
			ex.printStackTrace();
			throw new ValidationException( (ex instanceof ValidationException ? "" : "ValidationUtil Failure! " ) + ex.getMessage() );
		}
	}
	 ...
	 */
	protected static double getSizePercentAllowed( final BioModule module ) throws ConfigFormatException {
	...
	private static String fileNameToKey( final String fileName ) {
		...
		if( key.equals( fileName ) ) key = fileName.replaceAll( pipePrifix + "_[0-9]{4}[A-Za-z]{3}[0-9]{2}", "_DATE" );
		return key;
	}
	...
	private static ArrayList<String> getReportSet( final BioModule module ) throws Exception {
		...
		if( reportFeatures.isEmpty() ) reportFeatures.addAll( availableAttributes );
		else if( !availableAttributes.containsAll( reportFeatures ) ) {
	...
	/**
	 * Optional column in expectation file that stores the md5 sum of the output dir/file {@value #MD5}
	 */
	...
	/**
	 * Optional column in expectation file that measures output dir/file size {@value #SIZE}
	 */
	 ...
	 */
	protected static final String DISABLE_VALIDATION = "validation.disableValidation";
