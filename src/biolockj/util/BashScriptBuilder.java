...
public class BashScriptBuilder {
	 ...
	 */
	protected static List<String> buildExecuteFunction( final String script ) {
	...
	protected static List<String> getWorkerScriptLines( final List<String> lines ) {
		...
		for( final String line: lines )
			out.add( FUNCTION_EXECUTE + " \"" + line + "\" ${LINENO}" );
		out.add( "" );
		return out;
	}
	 ...
	 */
	protected static void writeScript( final BufferedWriter writer, final List<String> scriptLines ) throws IOException {
		int indentCount = 0;
		final List<String> lines = new ArrayList<>();
		try {
			for( String line: scriptLines ) {
				if( !line.trim().startsWith( PIPE_DIR ) && line.contains( Config.pipelinePath() ) ) 
					lines.add( line.replaceAll( Config.pipelinePath(), Matcher.quoteReplacement( "${" + PIPE_DIR + "}" ) ) );
				else lines.add( line );
			}
			...
			for( String line: lines ) {
				...
				int i = 0;
				while( i++ < indentCount ) line = Constants.INDENT + line;
