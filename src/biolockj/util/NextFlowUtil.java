...
public class NextFlowUtil
{
	...
	public static void buildNextFlowMain( List<BioModule> modules ) throws Exception
	{
	...
	protected static List<String> getNextFlowLines( List<BioModule> modules ) throws Exception
	{
		final List<String> lines = new ArrayList<>();
		lines.add( "// Nextflow main.nf generated by BioLockJ " + BioLockJUtil.getVersion() + " - Config: " + Config.getConfigFilePath() );
		lines.add( "" );
		lines.add( Constants.AWS_EFS_DIR + " = \"" + Config.requireString( Constants.AWS_EFS_DIR ) + "\"" );
		lines.add( Constants.PROJECT_PIPELINE_DIR + " = \"" + getProp( Constants.AWS_EFS_DIR ) 
	...
	{
		return CONTAINER + " '" + JOB_DEF + "://" + Config.requireString( Constants.AWS_STACK ) + "_"
				+ DockerUtil.getImageName( module.getClass().getSimpleName() ) + "_" + DockerUtil.getImageVersion( module ) + "'";
	}

	private static String getCpusLine( final BioModule module ) throws Exception
	{
		final String prop = module.getProperty( ScriptModule.SCRIPT_NUM_THREADS );
		final Integer cpus = Config.requirePositiveInteger( prop );
	...
	private static List<String> getInputLines( final BioModule module ) throws Exception
	{
		...
		lines.add( "val " + AWS_WORKER + " from Channel.watchPath( '" + getProp( Constants.PROJECT_PIPELINE_DIR ) 
			+ File.separator + module.getModuleDir().getName() + File.separator + ScriptModule.SCRIPT_DIR + "*.sh'" );
		return lines;
	}

	private static String getMemoryLine( final BioModule module ) throws Exception
	{
		final String prop = module.getProperty( Constants.AWS_RAM  );
		return MEMORY + " '" + Config.requireString( prop ) + "'";
	}
